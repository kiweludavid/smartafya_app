...
class _AppointmentsScreenState extends State<AppointmentsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (user.isDoctor) {
        sess = sess.where((s) => s.doctorId == user.id).toList();
        final ids = sess.map((s) => s.bookingId).where((id) => id.trim().isNotEmpty).toSet();
        final loaded = <BookingDto>[];
        await Future.wait(
          ids.map((id) async {
            try {
              loaded.add(await _api.fetchBookingById(id).timeout(const Duration(seconds: 6)));
            } catch (_) {}
          }),
        );
        books = loaded;
