...
class DoctorHomeDashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final SessionDto? nextSession = _resolveNextSession(sessions, now);
  ...
  /// Picks the session for the home "Next session" card: in-progress first, then
  /// the soonest future session. Past scheduled sessions are never shown here.
  static SessionDto? _resolveNextSession(List<SessionDto> sessions, DateTime now) {
    bool isTerminal(String st) {
      return st == 'completed' ||
          st == 'cancelled' ||
          st == 'no_show' ||
          st == 'no-show' ||
    ...
    bool isInProgress(SessionDto s) {
      final st = s.status.toLowerCase().trim();
      return st == 'in_progress' || st == 'in-progress';
    }

    bool isInSessionWindow(SessionDto s) {
      if (isTerminal(s.status.toLowerCase().trim())) return false;
      final dt = _parseLocal(s.scheduledAt);
      if (dt == null) return false;
      return !dt.isAfter(now) && dt.add(const Duration(hours: 2)).isAfter(now);
    }
    ...
    final inProgress = sessions.where(isInProgress).toList()
      ..sort((a, b) => _cmpSchedule(a.scheduledAt, b.scheduledAt));
    if (inProgress.isNotEmpty) return inProgress.first;

    final activeWindow = sessions.where(isInSessionWindow).toList()
      ..sort((a, b) => _cmpSchedule(a.scheduledAt, b.scheduledAt));
    if (activeWindow.isNotEmpty) return activeWindow.first;
  ...
  static int _cmpSchedule(String? a, String? b) {
    final da = _parseLocal(a);
    final db = _parseLocal(b);
    if (da == null && db == null) return 0;
    if (da == null) return 1;
  ...
  static String _displayStatus(SessionDto s) {
    ...
    if (st == 'in_progress' || st == 'in-progress') return 'Ongoing';
    ...
    if (st == 'scheduled') {
      ...
      final now = DateTime.now();
      // Ongoing only after start time and still within the session window.
      if (!dt.isAfter(now) && dt.add(const Duration(hours: 2)).isAfter(now)) {
        return 'Ongoing';
      }
